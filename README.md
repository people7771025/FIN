...
2. 雙擊開啟瀏覽器
3. 進度、書籤、設定自動保存在瀏覽器 localStorage
...
- **Part Ⅷ 從框架到實戰** 🎯：完整端到端走查一家公司（六步 SOP 跑完）、估值落地（DCF／PEG／本益比區間／殖利率法算出一個數）、去哪抓與怎麼讀真實財報（MOPS／SEC EDGAR 操作、中英科目對照、同業與多期資料工序）
...
- **看財報工作底稿（Worksheet）** — 六步體檢可填、可標紅旗、自動存、可匯出文字
